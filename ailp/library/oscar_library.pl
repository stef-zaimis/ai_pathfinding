...
possible_query(internal_topup_energy, [_Agent,_ChargStation]).  % ( internal_topup_energy, [2, c(1)]).
possible_query(internal_agent_do_moves, [_Agent, _Path]).       % ( internal_agent_do_moves, [ 1, Path]).
possible_query(internal_agents_do_moves,[_Agents,_Moves]).      % ( internal_agents_do_moves, [ [1] , [move]])
...
% This allows you to join the game using multiple agents
join_game([],0) :- !.
join_game(Agents, C):-
  C > 0,
  ( \+query_world(game_status,[running]) ->
    (query_world(internal_join_game, [Agent]),
     assert(ailp_internal(agent(Agent))),
     C1 is C-1,
     join_game(OtherAgents,C1),
     append([Agent],OtherAgents,Agents)
    )
  ; otherwise                            -> format('Cannot join! Game has already started')
  ).
...
% Returns an agent belonging to the terminal arbitrarily
my_agent(Agent):-
  ailp_internal(agent(Agent)),!.

% Returns all agents belonging to the terminal.
my_agents(Agents):-
  findall(Agent, ailp_internal(agent(Agent)), Agents ).
...
shell2query(energy,(my_agent(Agent),get_agent_energy(Agent,E)),both(current_energy(E))).
shell2query(topup(S),(my_agent(Agent),agent_topup_energy(Agent,S)),agent(topup)).
...
shell2query(maze,solve_maze,ok).
