...
find_submission(Assignment, Submission) :-
  ...
  ; Assignment='oscar'       -> find_file('cw_', Files, Submission)
  ; Assignment='wp'          -> find_file('cw_wp_', Files, Submission)
  ; Assignment='maze'        -> find_file('cw_maze_',Files, Submission)
  ).
...
translate_input_options(Args, Assignment_name, Part) :-
  ...
  ; Args=['cw','part1'|[]] -> Assignment_name='oscar',Part=1
  ; Args=['cw','part2'|[]] -> Assignment_name='oscar',Part=2
  ; Args=['cw','part3'|[]] -> Assignment_name='maze',Part=3
...
read_in_choice(Assignment_name, Part) :-
  ...
  write('  e.g. `lab identity` or `cw part2`'),nl,
...
get_assignment_details :- 
    ...
    ; Assignment_part=1 -> use_module(assignment_library('wp_library')),
                           use_module(assignment_library('oscar_library'), [part_module/1,shell/0,start/0,stop/0,
                                                                            ...
                                                                            reset_game/0,map_adjacent/3,map_distance/3,say/2,ailp_grid_size/1]
                                     ),
                           use_module(assignment_library('game_predicates'), [lookup_pos/2,get_agent_energy/2,get_agent_position/2,agent_do_moves/2,agent_check_oracle/2,agent_ask_oracle/4,agent_topup_energy/2]),
                           find_submission('oscar', OscarSubmission),
                           load_files([assignment_root(OscarSubmission)], [silent(true)])
    ; Assignment_part=2 -> use_module(assignment_library('wp_library')),
                           use_module(assignment_library('oscar_library'), [part_module/1,shell/0,start/0,stop/0,
                                                                            my_agent/1,
                                                                            leave_game/0,join_game/1,start_game/0,
                                                                            reset_game/0,map_adjacent/3,map_distance/3,say/2,ailp_grid_size/1]
    ...
    ; Assignment_part=3 -> use_module(assignment_library('wp_library')), 
                           use_module(assignment_library('oscar_library'), [part_module/1,shell/0,start/0,stop/0,
                                                                            my_agent/1,my_agents/1,
                                                                            leave_game/0, leave_maze/1, join_game/1,join_game/2,start_game/0,
                                                                            reset_game/0,map_adjacent/3,map_distance/3,say/2,ailp_grid_size/1,agent_adjacent/3]
                                     ),
                           use_module(assignment_library('game_predicates'), [lookup_pos/2,get_agent_energy/2,get_agent_position/2,agent_do_moves/2,agents_do_moves/2]),
                           find_submission('maze', MazeSubmission),
                           load_files([assignment_root(MazeSubmission)], [silent(true)]),
                           find_submission('oscar', OscarSubmission),
                           load_files([assignment_root(OscarSubmission)], [silent(true)]),
